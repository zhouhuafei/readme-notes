...
* 微盟 - 上海向心云网络科技有限公司（SaaS电商）（线上门店，选品之后，商品是门店商品，库存是门店库存）
  - 草动商城后台管理系统：Vue全家桶 + Element-UI
    ...
    - 值得一说的事：引入了commit提交规范、引入了GitLab的CI/CD自动化打包、引入了企微机器人接口报警、促进了后端接口规范、引入了微前端、引入了TS。获得了2019年的年度优秀员工。
    - 为什么引入微前端：方便局部重构（Vue2迁移到Vue3）、功能模块拆分（例如智能营销模块是由另外的团队开发）、子应用独立开发独立部署（子应用崩了不会影响到主应用）、技术栈可以不用统一（建议统一）。
    - 前端页面的发布由前端自己管理：非覆盖式发布、OSS对象存储、CDN缓存。

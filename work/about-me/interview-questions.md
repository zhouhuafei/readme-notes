...
* 什么是变量的作用域链？`先看当前作用域中有没有某个变量，如果没有则去上级作用域中进行查找，依次类推，形成作用域链`。
#### 预解析？
...
* call和apply以及bind的区别？`call和apply入参形式不同、bind返回一个新函数`。
  ...
  ```
#### 怎么理解js的单线程？线程空闲？异步？
* setTimeout倒计时为什么会出现误差？`单线程、线程空闲、异步`。
#### 宏任务与微任务？
* 常用的宏任务？`script、setTimeout、setInterval、requestAnimationFrame`。
* 常用的微任务？`Promise.then、Promise.catch、Promise.finally`。
* 宏任务与微任务的执行顺序？`执行宏任务途中，如果遇到了微任务则放入微任务队列。当前宏任务执行完毕，会查看微任务队列有无微任务，如果有，则把微任务队列里的微任务清理干净`。
#### 应用？

...
* 选择器`+`和`~`以及`>`的区别？`后面相邻兄弟选择器，后面兄弟选择器，直属子元素选择器`。
  - `div + p`：div后面相邻的兄弟元素p，div前面相邻的兄弟元素p不会被选中。
  - `div ~ p`：div后面所有的兄弟元素p，div前面所有的兄弟元素p不会被选中。
...
* 小程序生成某个页面二维码的前提是？`不需要前提，小程序可以尚未发布，也可以不存在对应页面`。
* 小程序生成的页面二维码，扫码能跳开发版或者体验版的小程序么？`能`。
* 怎么在开发阶段保证二维码能正常生成？`支持直接生成，如果后端接口不智能，就做个scene中转页`。

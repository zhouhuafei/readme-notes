...
* 其实本身还是1px，只不过截图到ps软件里，被放大了倍数。
* 肉眼看起来像2px，其实是高倍屏看起来更清晰罢了。实际2倍屏下还是1个像素的设备独立像素，不过占了2个像素的设备物理像素，所以看起来更清晰。
* 设备像素比为2时，1个设备独立像素点，会占4个设备物理像素（pt）。
* 所以一个1*1的设备独立像素方格，在2倍屏下会展示成2*2的设备物理像素方格。
* 这个在网页里可以使用dpr缩放进行解决。可以把高倍屏下的1px缩放成0.5px。看起来就没有那么粗了。如果详情页有从其他站点拿过来的带px单位的数据。则把window.devicePixelRatio设置成1即可。总要有所取舍，毕竟数据比边框重要。
...
* 例如手机端一张320px的图片，如果我使用的是960px的图片，设计图里设计的960px应该是等于设备物理像素的960pt的。
* 1倍屏320px的图片，图片3px的css像素，等于1像素的设备独立像素，1像素的设备独立像素等于1像素的设备物理像素，等比缩小，图片不会模糊。
* 2倍屏320px的图片，图片3px的css像素，等于1像素的设备独立像素，1像素的设备独立像素等于2像素的设备物理像素，等比缩小，图片不会模糊。
* 3倍屏320px的图片，图片3px的css像素，等于1像素的设备独立像素，1像素的设备独立像素等于3像素的设备物理像素，没有缩放，图片不会模糊。
* 所以手机端一般是使用2倍图或者3倍图。

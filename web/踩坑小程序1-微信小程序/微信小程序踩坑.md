...
#### 名词解释
* 网页：包含外部浏览器的网页和微信内置浏览器的网页。
* 微信内：微信的聊天框和微信的朋友圈都算是微信内。
#### URL Scheme 和 URL Link 和 Short Link 的区别？
* URL Scheme：适用于短信、邮件、外部网页、微信内等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 下述均为亲测 - 在不同的场景中使用：
  ...
  - 补充：上述外部网页应该改为网页，因为我亲测后发现，在微信的内置浏览器网页中使用URL Scheme时，Android和IOS均可以跳转。
* URL Link：适用于短信、邮件、网页、微信内等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 下述均为亲测 - 在不同的场景中使用：

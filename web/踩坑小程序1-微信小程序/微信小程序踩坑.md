...
#### 名词解释
* 浏览器：包含外部浏览器和微信内置浏览器。...TODO
* 微信内：微信的聊天框和微信的朋友圈都算是微信内。...TODO
#### URL Scheme 和 URL Link 和 Short Link 的区别？
* URL Scheme：适用于短信、邮件、外部浏览器、内置浏览器、微信内等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 亲测发现：不存在只能被1个用户访问的限制。
  - 亲测在不同的场景中使用：
    - web-view内嵌h5时 - Android可以正常点击 - IOS点击没反应
    - 在外部浏览器中或微信内置浏览器中使用时 - Android和IOS均可以正常使用
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 案例：`weixin://dl/business/?t=MEtcwXfPw9p`
* URL Link：适用于短信、邮件、外部浏览器、内置浏览器、微信内等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 亲测发现：在外部浏览器中访问时只能被1个用户访问。在内置浏览器中（...TODO）访问时不存在只能被1个用户访问的限制。
  - 亲测在不同的场景中使用：
    - web-view内嵌h5时 - Android和IOS均无法正常使用 - 会缺失 打开小程序 按钮
    - 在外部浏览器中或微信内置浏览器中使用时 - Android和IOS均可以正常使用
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 案例：`https://wxaurl.cn/XesI4uTIoIt`
  - 补充：URL Link是由官方提供的h5中转页，页面布局和云开发h5官方提供的页面布局是一样的。如果想定制h5中转页，建议使用云开发h5跳小程序。
* Short Link：适用于微信内拉起小程序的业务场景。可以选择生成到期失效和永久有效的小程序短链。到期失效的短链其有效期是30天。单日生成上限为100万。永久有效的总上限为10万个。
  ...
  - 亲测在不同的场景中使用：
    ...
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 案例：`#小程序://High购节/我的/DGIKasduAblpCMe`

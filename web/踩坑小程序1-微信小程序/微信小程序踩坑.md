# 微信小程序
![图片加载中...](./images/wx1.png)
![图片加载中...](./images/wx2.jpg)
...
* `JSBridge`是啥？
    - 参考资料：https://juejin.im/post/593b52d4ac502e006b2d54bc
    - 参考资料：https://developers.weixin.qq.com/ebook?action=get_post_info&docid=0008487094c6b84b0086d68a551c0a&highline=jsbri
    - WeiXinJSBridge 兼容层，主要提供了调用（invoke）和监听（on）这两种方法。
    - 待续...
* 微信小程序的基础库是啥？
    ...
    - 微信小程序的基础库就是`微信小程序的SDK`，官方文档称其为`基础库`。
...
* 在浏览器中js可以操作BOM和DOM，是因为在浏览器环境中，javascript的三大组成部分是：ECMAScript、DOM、BOM。浏览器内置了BOM和DOM的API供javascript调用。
  ...
  - 渲染引擎负责对网页的语法(html、css)进行解释并渲染网页。

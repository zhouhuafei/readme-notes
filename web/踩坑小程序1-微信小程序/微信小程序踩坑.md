...
* 下拉刷新时(疑惑)，data上的某些数据要设置成默认状态。然后重新执行```onLoad(this.data.options)```。
    - 有定时器要清理掉定时器。
    - 有分页数据则要重置为默认数据。
    - 有列表数据则要重置为空。
* 下拉刷新时(厉害)，data上的数据，感觉全部都设置成默认数据最安全。
    - data上全都默认为null。或者data上不设置数据。
    - 在onLoad的时候时候所有默认值。注：如果有定时器要先清理一下定时器。
    - 下拉刷新的时候重新执行```onLoad(this.data.options)```。

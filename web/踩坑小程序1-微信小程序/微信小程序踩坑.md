...
#### 案例：`<BaseNoData style="display: block;background: #f00;color: #fff;">门店已打烊</BaseNoData>`
#### 发现：上述如果没有display: block;则其他样式不会生效。
#### 总结：经过多次测试，我总结了以下解决方案。
* 方案1：把BaseNoData这个标签主动变成块元素或被动变成块元素。
* 方案2：把BaseNoData这个标签主动变成内联块元素或被动变成内联块元素。
* 方案3：把BaseNoData这个组件内部的根节点变成内联元素或内联块元素。
* 方案4：给BaseNoData这个组件外部的父级设置样式。

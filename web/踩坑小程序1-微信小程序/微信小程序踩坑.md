...
* 方式一：
    - 点击圆圈时。会触发。page的onHide。
    - 然后通过手机的菜单再次进入时，会触发。app的onShow。然后是page的onShow。如此操作流程不会触发页面的onUnload。
* 方式二：
    - 先把页面转发出去。此时。会触发。page的onHide。app的onShow。然后是page的onShow。
    - 然后点击圆圈。此时。会触发。page的onHide。
    - 然后通过分享卡片点进去。此时。会触发。app的onShow。然后是page的onUnload。然后是page的onLoad。然后是page的onShow。
...
* app.js中的onShow钩子先执行且钩子中的代码执行完之后，页面最顶部代码才执行。
* app.js
```
App({
  onShow (opts) {
...
* 解决方案1(局部处理)：针对需要授权的地方，进行按钮处理。如果没授权，则跳授权页，如果授权了，才继续执行。应该针对微信的四种跳转进行二次封装，如此处理起来会方便许多。
```
module.exports = {
  toAuthorization () { // 如果没头像或者没昵称则跳授权页
    if (!wx.getStorageSync('nickName') || !wx.getStorageSync('headUrl')) {
      const { fullPath } = getCurrentPage()
      wx.reLaunch({ url: `/pages/authorization/authorization?refererUrl=${encodeURIComponent(fullPath)}` })
      return true
    }
    return false
  }
...
wx.onAppRoute((route) => {
  ...
  if (pageIndex === -1) {
    ...
    // 在授权页点击取消之后，则会跳回授权页上带的参数(refererUrl)页或首页，并在此定时器触发时，没满足条件再次跳入授权页。
    // 为了防止重复跳入授权页，授权点取消直接回首页。并提示文案：确认要取消么？取消之后无法体验完整的小程序！

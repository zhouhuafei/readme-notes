...
* web-view跳其他小程序。
  ...
  - 方案3 - 用过：h5和小程序通信，使用jssdk的postMessage让h5和小程序通信，不需要调用wx.config，但是存在触发时机问题。
    - 如果涉及到用户主动触发，则实现不了，因为在h5上的点击，不算是小程序的主动触发行为。
    - 所以内嵌h5，无法触发小程序卡片转发，只能引导用户主动转发。
    - 订阅消息等需要用户主动触发的能力亦无法使用。
  - 方案4 - 推测：使用开放标签，开放标签需要调用wx.config，固没被采用。
  ...
  - 方案8 - 代理：使用代理服务器规避jssdk通信配置问题。例如使用websocket和小程序通信。

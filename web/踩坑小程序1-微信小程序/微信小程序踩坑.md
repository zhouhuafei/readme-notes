...
* 注意：使用`wx.request`发送请求时，`wx.request`会对请求体中的值进行`encodeURIComponent`编码。
  - `wx.request`会对`GET`和`POST`请求都进行`encodeURIComponent`编码。
  - `axios`只会对`GET`请求进行`encodeURIComponent`编码。
  - `xhr`本身默认不会进行编码。
  - 如果客户端进行了编码，则服务端拿到的值就是编码后的值，但是框架或中间件一般默认会对请求体中的值进行一次解码。
...
* 如果有tabBar。screenHeight是完整的屏幕高度。windowHeight是screenHeight减去tabBar高度再减去底部安全区域高度后的高度。
* 如果无tabBar。screenHeight是完整的屏幕高度。windowHeight高度和screenHeight高度相等。

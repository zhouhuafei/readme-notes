...
* ctx.scale真机上导致多次缩放。
  ...
  ```
* 微信小程序的`ctx.font`会穿透`ctx.save`和`ctx.restore`，导致`ctx.measureText`会根据上一次设置的`font`测量。
  - 微信小程序的api`ctx.setFontSize`同样存在上述问题。
  - 浏览器的`ctx.font`不会穿透`ctx.save`和`ctx.restore`。不存在上述问题。

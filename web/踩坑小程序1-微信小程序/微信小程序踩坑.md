...
* `JSBridge`是啥？
  - JSBridge 是js和Native之间的桥梁，是JS和Native之间的一种通信方式。
  - `wx`就是客户端暴露给小程序的JsBridge接口。而这个封装的JsBridge非常强大，它不仅仅支持toast，弹框等简单功能，甚至包括网络request请求，缓存操作，手机硬件设备蓝牙、重力感应等。
  - 按照上述言论，我感觉微信小程序的JSBridge就是基础库。
* 微信小程序的基础库是啥？

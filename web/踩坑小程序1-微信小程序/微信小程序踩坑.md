...
* canvas层级太高，覆盖了底部导航。
    ...
    - 其他：view组件无法覆盖原生组件，cover-view组件能覆盖原生组件，view组件层级高于cover-view组件时能覆盖cover-view组件。
* cover-view踩坑
...
```scss
.iPhoneXb {
  bottom: 34px !important; // iPhone X 距底差值是68rpx
}

.iPhoneXh {
  height: 34px !important; // iPhone X 距底差值是68rpx
}
```
* 怎么全局适配？
    - 1、制作一个悬浮的层，遮住底部的内容。

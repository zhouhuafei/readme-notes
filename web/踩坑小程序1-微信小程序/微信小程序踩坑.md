...
* 在浏览器中js可以操作BOM和DOM，是因为在浏览器环境中，javascript的三大组成部分是：ECMAScript、DOM、BOM。浏览器内置了BOM和DOM的API供javascript调用。
  - 浏览器内核分成两部分：渲染引擎和JS引擎。
  - 渲染引擎负责解析DOM文档和CSS规则并将内容排版到浏览器中显示有样式的界面，也有人称之为排版引擎，我们常说的浏览器内核主要指的就是渲染引擎。
  - JS引擎能够解释`javascript`代码，并且通过`DOM`接口和`CSSOM`接口来修改网页内容和样式信息，从而改变渲染结果。
* `Node.js`是一个基于`Chrome V8`引擎的`JavaScript`运行环境。
  - 此运行环境中没有BOM和DOM。

# 浏览器中的JavaScript、NodeJS中的JavaScrip、小程序中的JavaScript
> 参考资料：https://developers.weixin.qq.com/ebook?action=get_post_info&volumn=1&lang=zh_CN&book=miniprogram&docid=000a8806958588cb00862bd5851c0a
...
* 小程序中的JavaScript 是由ECMAScript 以及小程序框架和小程序 API 来实现的。同浏览器中的JavaScript 相比没有 BOM 以及 DOM 对象，所以类似 JQuery、Zepto这种浏览器类库是无法在小程序中运行起来的，同样的缺少 Native 模块和NPM包管理的机制，小程序中无法加载原生库，也无法直接使用大部分的 NPM 包。
...
* wx.reLaunch
  - 关闭所有页面，打开到应用内的某个页面。
  - wx.reLaunch会触发当前页面的onUnload(触发onUnload就不会触发onHide)，不会触发app.js的onLaunch。onLaunch生命周期在app.js中初次加载触发。
* wx.switchTab
...
* 在web-view中可以长按保存图片。也可以把图片转发给朋友。
...
* web-view跳其他小程序。
  ...
  - 方案3 - 亲测：h5和小程序通信，使用jssdk的postMessage让h5和小程序通信，不需要调用wx.config，但是存在触发时机问题。
  - 方案4 - 亲测：使用开放标签，开放标签需要调用wx.config，固没被采用。
  - 方案5 - 推荐：Android和IOS都可以对web-view中的小程序码，进行长按识别。
  - 方案6 - 亲测：使用web-view的bindload事件，当页面加载（load）完毕后会触发对应事件，在事件中可以获取到页面的完整路径。无法监听SPA应用的路由切换。
  - 方案7 - 引导：使用业务引导，在h5页面中做个复制按钮，引导用户复制`URL Scheme`的链接到微信聊天框中，然后直接点击链接进行访问。
  - 方案8 - 代理：使用代理服务器进行实时通信。例如h5使用websocket和小程序进行通信。
    - 即使能够实时通信，但若涉及到需要用户主动触发的api，则无法使用。因为在h5上的点击，不算是小程序的主动触发行为。
    - 所以web-view内嵌h5，无法触发小程序卡片转发，只能引导用户主动转发。订阅消息和授权头像昵称等需要用户主动触发的能力亦无法使用。
* web-view跳自家小程序，直接引入jssdk就可以，不需要调用wx.config，就可以使用文档中提供的api。
...
* web-view可以跳入同主体的公众号文章详情，不能跳入同主体的公众号文章列表，不同主体不能跳入。
...
* externalClasses很有局限性，无法覆盖组件内已有样式。【加!important可以覆盖】
  - 注意：外部样式类只作用于当前节点本身，无法控制其子节点。如果想要控制子节点，就要给子节点也设置外部样式类。
...
* html：`\n`和`<br />`在不同场景中渲染的结果并不相同。
  ...
  - 在`innerText`中`\n`会渲染成换行。多个`\n`会渲染成多个换行。打印看了一下，发现此时`\n`被转成了`<br>`（这是innerText的特性）。
  - 在`innerText`中`<br />`会渲染成`<br />`。多个`<br />`会渲染成多个`<br />`。
  - 使用pre标签对`\n`进行渲染，一定会渲染成多个换行。不管是使用`innerHTML`还是`innerText`。
...
wx.getSystemInfo({
  ...
  fail: () => {
    this.globalData.statusBarHeight = 40 // iPhone X (375 x 812 | Dpr:3)
...
* 补：并不是es6模板字符串具备让贪婪属性无效的能力，其实是因为元字符`.`的特性是查找单个字符，除了换行和行结束符。
  - 如果你想匹配包括换行符在内的任意字符，你可以使用`[\s\S]`或`[\d\D]`或`[\w\W]`这样的正则表达式。
* `/<[^<>]+>/g`：正则匹配HTML标签。正则匹配标签。
  - `[^<>]+`可以跨行匹配。
...
# 第三方插件`wxParse`和官方组件`rich-text`的区别。
...
* 问：内部的图片处理区域，怎么做到中间透明，图片上下超出区域覆盖遮罩的？
* 答：canvas一层一层局部画出来的遮罩。
> https://github.com/wx-plugin/image-cropper
* 问：内部的图片处理区域，怎么做到中间透明，图片上下超出区域覆盖遮罩的？
* 答：html一层一层局部画出来的遮罩。
> 如果是我，我会怎么做？
* 我会用box-shadow模拟遮罩。
    - 第一层：使用方块区域的box-shadow模拟遮罩。
    - 第二层：图片。
    - 第三层：全屏透明遮罩。
    - 第四层：再覆盖一个透明方块遮罩覆盖住方块区域。
    - 最后是：操作透明方块遮罩，让图片做对应的位移。

# 小程序使用分包
> 文档：https://developers.weixin.qq.com/miniprogram/dev/framework/subpackages/basic.html
* 注意：分包之间的文件，不能互相引用。
* 注意：`tabBar`页面必须在`app`(主包)内。
* 注意：页面的跳转路径，要拼全。案例：`/pages-subpackages/cus/pages/cus-m/index`。
* 总结：分包的使用，还是挺容易的，根据文档进行配置即可。文档中也有示例代码包，下载下来看一下就明白了。
* 示例：
```
{
  "subpackages": [
    {
      "root": "pages-subpackages/cus/",
      "pages": [
        "pages/cus-m/index",
        "pages/cus-m-detail/index",
        "pages/cus-m-label/index",
        "pages/cus-m-label-search/index",
        "pages/cus-m-label-detail/index"
      ]
    }
  ]
}
...
# app.json permission配置
> 文档：https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html#permission
* 示例：
```
{
  "permission": {
...
# textarea的placeholder层级太高
* 弹层出现时，把`placeholder`动态置空，暂时能解决这样的问题。

# scroll-view给元素加了阴影box-shadow 为什么上边像被切割了一样？
* 因为它自带了`overflow:hidden`，`swiper`也一样，建议在`scroll-view`内套一层，在这一层上设`padding`。
* scroll-view组件的滚动效果可以被mask遮罩挡掉。view组件用css写的滚动也会被mask遮罩挡掉。页面自带的滚动不会被mask遮罩挡掉。
* 阻止小程序页面滚动：给mask遮罩增加`catchtouchmove`事件即可。不管事件里有无内容，页面滚动都会被阻止。
* 阻止浏览器滚动：
```javascript
const obj = {
    // 阻止冒泡
...
# 二维码的参数是scene，转发卡片的参数不是scene。能统一么？
* 能统一。转发的时候，模拟scene数据格式组成数据即可。

# 微信小程序线上怎么调试？
* 只需要在开发版本或体验版本中打开调试，然后再进入线上版本则可以显示出调试按钮。

# 长列表卡顿
* 原因：图片太多，图片太大。
* 思路：如果技术上不好实现，那么就从业务上去解决。
* 方案：1、使用七牛的图片参数`?imageView2/2/w/240`，进行图片压缩。2、把列表中的图片减少，当用户点了展开，才渲染对应项的其他图片。3、分页一页由之前的10个更换为5个。
* 测试：经测试，卡顿、闪退的问题就这样被解决了。虽治标不治本。但是足够了。

# 使用微信内置地图查看位置
> 注：经纬度需要是数字类型。不能是字符串类型。
```
wx.openLocation({
...
# 微信开发者工具的增强编译
> 勾上增强编译则可以使用`async`和`await`。但是小程序的API依然不是Promise的。
...
if (updateManager) {
  if (updateManager.onUpdateReady) {
...
# 在小程序的自定义组件中使用canvas不生效
* 原因：canvas组件注册无效，这个因为createCanvasContext方法是有两个参数，在page页面默认传了一个this，在组件里面需要手动传this。
* 解决：`const ctx = wx.createCanvasContext(canvasId, this)`。
* `wx.canvasToTempFilePath`也存在这个问题。在自定义组件中使用时需要传入第二参数this`wx.canvasToTempFilePath(options, this)`。

# 局部mixins生命周期
* src/mixins/life-cycle.js
```
export default function ({ onLoad, onHide, onShow }) {
  return {
    onLoad (options) {
    ...
    onShow () {
      ...
      wx.$sr.report({ type: 4, props: { is_newly_open: false } })
    }
  }
}
```
...
```
import mixinsLifeCycle from '../../mixins/life-cycle.js'

Page({
  ...mixinsLifeCycle.call(this, {
    onShow () {
      // 业务逻辑
    }
  })
...
# 小程序强制横屏
* 在`app.json`或页面的`json`文件中配置`pageOrientation`为`landscape`即可。

# 使用命令行上传
* https://www.npmjs.com/package/miniprogram-ci

# 页面、父组件、子组件的渲染顺序。生命周期的触发顺序。
#### 在Vue2中 - 不管组件有无入参，生命周期总是会按照下述顺序触发。在Vue3中亦如此。
```
page beforeCreate
page created
page beforeMount
...
childComponent mounted 子组件先渲染
parentComponent mounted 父组件后渲染
page mounted 页面最后渲染
```
#### 在微信原生小程序中 - 不管组件有无入参，生命周期总是会按照下述顺序触发。在uni-app的Vue2中亦如此。
```
childComponent created
parentComponent created
parentComponent attached
childComponent attached
page onLoad
page onShow
childComponent ready 子组件先渲染
parentComponent ready 父组件后渲染
page onReady 页面最后渲染
```
* 值得注意的是小程序的组件可以在pageLifetimes中监听页面的生命周期且触发时机优于页面的生命周期。
#### 在uni-cli的Vue3中 - 组件有传参时，生命周期的触发顺序会有所变化。在uni-app的Vue3中亦如此。
* 当组件无入参时
```
page onLoad
parentComponent onLoad
childComponent onLoad
page onShow
parentComponent onShow
childComponent onShow
childComponent onBeforeMount
childComponent onMounted
childComponent onReady 子组件先渲染
parentComponent onBeforeMount
parentComponent onMounted
parentComponent onReady 父组件后渲染
page onBeforeMount
page onMounted
page onReady 页面最后渲染
```
...
childComponent onBeforeMount
childComponent onMounted
childComponent onReady 子组件后渲染
parentComponent onBeforeMount
parentComponent onMounted
parentComponent onReady 父组件最后渲染
```
* 当父组件无入参，子组件有入参时。
```
page onLoad
...
parentComponent onBeforeMount
parentComponent onMounted
parentComponent onReady 父组件先渲染
page onBeforeMount
page onMounted
page onReady 页面后渲染
childComponent onBeforeMount
childComponent onMounted
childComponent onReady 子组件最后渲染
```
* 当父组件有入参，子组件也有入参时。
```
page onLoad
...
# wx.showLoading 和 wx.showToast
* wx.showToast触发时wx.showLoading会无效。
* wx.showLoading触发时wx.showToast会无效。

# 组件套组件 - `addGlobalClass: true`无效
* 组件样式隔离：https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/wxml-wxss.html#%E7%BB%84%E4%BB%B6%E6%A0%B7%E5%BC%8F%E9%9A%94%E7%A6%BB
  - 组件套组件 - `addGlobalClass: true`无效
* 外部样式类：https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/wxml-wxss.html#%E5%A4%96%E9%83%A8%E6%A0%B7%E5%BC%8F%E7%B1%BB
  - 这是方案1 - 我没选方案1
...
* 获取进入小程序时的options，比app.js里的onLaunch和onShow中形参上的options好用。
* 可以用以统计进入小程序的场景。
* onLaunch中options上的scene是不精准的。
  - 因为如果小程序不关闭，通过别人的卡片再进入是不会触发onLaunch的。
  - 使用`wx.getEnterOptionsSync()`获取到的就是精准无误的。

# 不同机型-同样是用户拒绝手机授权-返回的错误文案居然不一样
* 无奈啊。

# 移动应用APP
* APP拉起小程序功能（主动）：https://developers.weixin.qq.com/doc/oplatform/Mobile_App/Launching_a_Mini_Program/Launching_a_Mini_Program.html
* 小程序打开APP（被动）：https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/launchApp.html
  - 2021年5月27日后：通过场景值1036（从app分享卡片）打开的小程序，不再可以打开app。
  - 2021年5月27日后：通过场景值1069（从app拉起）打开的小程序，仍然可以返回app。
* 微信内置浏览器跳转APP功能（主动）：https://developers.weixin.qq.com/doc/oplatform/Mobile_App/WeChat_H5_Launch_APP.html

# h5跳小程序
* 微信内置浏览器h5跳小程序（开放标签）：https://developers.weixin.qq.com/doc/offiaccount/OA_Web_Apps/Wechat_Open_Tag.html#%E5%BC%80%E6%94%BE%E6%A0%87%E7%AD%BE%E8%AF%B4%E6%98%8E%E6%96%87%E6%A1%A3
* 微信外部浏览器h5跳小程序（URL Scheme）：https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/url-scheme.html
  - 补充：亲测后发现，微信内置浏览器h5跳小程序，使用URL Scheme如今也已经行得通了，Android和IOS均可以正常使用。
* 云开发h5跳小程序（开放标签和URL Scheme的结合）：https://developers.weixin.qq.com/miniprogram/dev/wxcloud/guide/staticstorage/jump-miniprogram.html
#### URL Scheme 和 URL Link 和 Short Link 的区别？
> 下述的名词`微信内`是指微信的聊天框和微信的朋友圈，不包含微信内置浏览器。
* URL Scheme：适用于短信、邮件、外部浏览器、微信内置浏览器、微信聊天框等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 亲测在不同的场景中使用：具体如下↓
    - web-view内嵌h5时 - Android可以正常点击 - IOS点击没反应
    - 在外部浏览器中或微信内置浏览器中使用时 - Android和IOS均可以正常使用
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 链接格式：`weixin://dl/business/?t=MEtcwXfPw9p`
  - 访问限制：从外部浏览器中访问时只能被1个用户访问。在微信内置浏览器中访问时不存在只能被1个用户访问的限制。
    - 在微信聊天框中点击URL Scheme时，会先打开微信内置浏览器，再出现弹窗询问。
* URL Link：适用于短信、邮件、外部浏览器、微信内置浏览器、微信内等拉起小程序的业务场景。有效期最长30天。单日生成上限为50万。无总上限。
  - 亲测在不同的场景中使用：具体如下↓
    - web-view内嵌h5时 - Android和IOS均无法正常使用 - 会缺失 打开小程序 按钮
    - 在外部浏览器中或微信内置浏览器中使用时 - Android和IOS均可以正常使用
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 链接格式：`https://wxaurl.cn/XesI4uTIoIt`
  - 补充说明：URL Link是由官方提供的h5中转页，页面布局和云开发h5官方提供的页面布局是一样的。如果想定制h5中转页，建议使用云开发h5跳小程序。
  - 访问限制：从外部浏览器中访问时只能被1个用户访问。在微信内置浏览器中访问时不存在只能被1个用户访问的限制。
    - 在微信内点击URL Link时，会先打开微信内置浏览器，然后出现h5中转页，点击中转页内的`打开小程序`按钮后会出现弹窗询问。
* Short Link：适用于微信内拉起小程序的业务场景。可以选择生成到期失效和永久有效的小程序短链。到期失效的短链其有效期是30天。单日生成上限为100万。永久有效的总上限为10万个。
  - 亲测在不同的场景中使用：具体如下↓
    - web-view内嵌h5时 - Android和IOS均无法正常使用 - 点击均没反应
    - 在外部浏览器中或微信内置浏览器中使用时 - Android和IOS均无法正常使用 - 点击均没反应
    - 在微信聊天框中或朋友圈文案中使用时 - Android和IOS均可以正常使用
  - 链接格式：`#小程序://High购节/我的/DGIKasduAblpCMe`
  - 访问限制：只能在微信内使用，不存在只能被1个用户访问的限制。
    - 在微信内点击Short Link时，不会打开微信内置浏览器，会直接出现弹窗询问。

# 短信跳小程序
* 文档：https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/sms.html
* 本质：短信里是h5页面路径链接，点击链接进入h5，通过h5中转跳入小程序。
  - 外部浏览器中的h5需要使用`URL Scheme`来实现跳转。
  - 微信内置浏览器中的h5需要使用`开放标签`来实现跳转（使用URL Scheme如今也已经行得通了，Android和IOS均可以正常使用）。
#### 无印良品MUJI会员小程序，短信跳小程序案例
* 短信内容：【MUJI无印良品】尊敬的会员，感谢您对MUJI品牌的喜爱与支持。诚邀您参加2025年春夏商品展示会，抢先预览体验品牌新品。名额有限，报满即止rch5.cn/3/sy83 。拒收请回复R
* 访问网址：rch5.cn/3/sy83
* 网址源码：./smsJumpMp.html
* 使用场景：在短信中点击链接可以跳入微信小程序、在微信中点击链接可以跳入微信小程序、在第三方应用例如飞书中点击链接可以跳入微信小程序

# 怎么判断当前小程序是体验版本还是正式版本还是开发版本？
```javascript
console.log('版本：', __wxConfig.envVersion)
console.log('版本：', wx.getAccountInfoSync().miniProgram.envVersion)
// 版本：'develop' // 开发版
// 版本：'trial' // 体验版
// 版本：'release' // 正式版
```
...
# 微信小程序canvas海报文字渲染偶尔异常
* 表现形式为：文字颜色偶尔丢失、文字位置偶尔丢失、文字大小偶尔丢失。
* 排查发现是：`ctx.draw`回调里直接立即调用`wx.canvasToTempFilePath`导致的。
* 解决方案为：`ctx.draw`回调里使用`setTimeout`延迟`100毫秒`后再调用`wx.canvasToTempFilePath`即可解决问题。
...
```
* 搜索关键字：上传文件封装、上传图片封装、input重置、file重置、value重置、置空、触发、change、onchange。
* 相比较于直接使用input类型为file的html标签，创建标签的方式不用担心选择同一张图片不会触发change的问题。
* input类型为file时选择同一张图片不会触发change。
  - 将文件域的value值设置为空可以解决此种问题。置空之后，下次选择同一张图片依然可以触发change事件。
  - 不管input的类型是file还是text，使用js将文件域的value值设置为空都不会触发change事件。

# 自定义tabBar方案
* 1. 使用底部悬浮自己制作。自己跳对应的页面。【弊端：跳转的体验不好。】
* 2. 使用底部悬浮自己制作。tabBar相关的页面做成一个单页。【弊端：页面代码太多。】
* 3. 使用官方的自定义tabBar。【弊端：会闪烁。体验不好。】
* 4. 先定义几个固定的tabBar，然后在页面上使之隐藏。然后配合自己制作的底部悬浮进行渲染，这样就可以使用switchTab了，体验也非常OK。【奇思妙想】

# 官方的自定义tabBar闪烁怎么解决？没有很好的解决方案！
* 社区讨论：https://developers.weixin.qq.com/community/develop/doc/0000ac8e944a4069240a0efb95b800?highLine=tabbar%25E9%2597%25AA%25E7%2583%2581
* 社区讨论：https://developers.weixin.qq.com/community/develop/article/doc/000a2a6ccb0d48f44e7bc612c56013
* 官方文档：https://developers.weixin.qq.com/miniprogram/dev/framework/ability/custom-tabbar.html

# 小程序如何控制不能二次转发或者分享？私密消息！
* 社区讨论：https://developers.weixin.qq.com/community/develop/doc/000620174fcd20f5e84b9abdb51c00?highLine=%25E7%25A6%2581%25E6%25AD%25A2%25E4%25BA%258C%25E6%25AC%25A1%25E8%25BD%25AC%25E5%258F%2591
...
* screenHeight是完整的屏幕高度。windowHeight是可使用窗口高度。
* 如果是自定义的导航栏、是自定义的tabBar、则两者高度相等。反之则screenHeight = windowHeight + 非自定义的导航栏高度 + 非自定义的tabBar高度 + 底部安全区域高度。

# tabBar高度
* 小程序原生tabBar高度是48px。

# 心得
* wx.api调用失败了应该用console.log打印个日志，方便排查错误。

# CDP-LinkFlow的小程序埋点是怎么做到监听页面的onShow、onHide等页面级方法的？
* 查看源码，发现是通过重写Page方法实现的。

# 微信开放平台第三方代实现小程序业务
* 一文了解第三方授权
  - https://developers.weixin.qq.com/community/business/doc/00062ea2c48a906a258caca215100d
* 平台型服务商代实现企业官网小程序（服务器版）
  - https://developers.weixin.qq.com/community/business/doc/0002ec37e8ccd80a49da9851f5b80d
* 平台型服务商代实现企业官网小程序（云开发版）
  - https://developers.weixin.qq.com/community/business/doc/0000ee798ecf28ca48da74ced5100d

## new Date兼容IOS - 否则会返回Invalid Date
```javascript
function NewDate (str) {
  if (str.replace) str = str.replace(/-/gi, '/')
  return new Date(str)
}
```
...
## wx.reLaunch会重置全局变量么？
* wx.reLaunch不会重置全局变量！
* wx.reLaunch不会触发app.js的onLaunch！

## scroll-view不支持flex怎么办？
* 给scroll-view组件增加enable-flex属性即可！

## 扫普通链接二维码打开小程序
* https://developers.weixin.qq.com/miniprogram/introduction/qrcode.html

## titleBarHeight
* 标题区域高度 - 官方给的高度是44px - 不存在误差
* 底部安全区域 - 官方给出的高度是34px - 可以使用safe-area-inset-bottom进行适配
...
## 小程序订单中心path设置
* 若审核不通过，可以联系客服。提供appid给客服，并给客服说此小程序是静默登录，拿到openid就算是登陆了，不需要授权手机号就可以登录。
* 说明完毕之后，客服会让你重新提审并帮忙附加说明，然后就可以审核通过了。

## 问题：即使组件存在样式隔离，但是，如果你在组件内使用text选择器给text标签设置样式，则会影响到页面上的text标签样式。
#### 方案：你应该给text标签设置一个class，然后使用class选择器去设置text的样式。
#### 应是标签选择器，在组件内使用时，都会存在类似的问题，固建议使用class选择器去设置样式。

## 微信小程序，标签上，style中的rpx会被自动转换为px。但是在真机上，转换时会出问题。即使是通过wxss设置的rpx，亦会存在同样的问题。
> 关键词搜索：rpx转px、rpx自动转px、rpx会被自动转换为px
#### 问题1-01：给标签设置left:0rpx，在我的小米14手机上，会被转换为1px。400/750*0等于0，即等于0时，会转换为1。
* 问题：使用不当，会导致，全屏的开屏页，出现白边。
* 注意：给标签设置left:0时不会出现问题，只要单位是rpx就会存在问题。
* 小米14：screenWidth是400，screenHeight是874，pixelRatio是3。
#### 问题1-02：给标签设置left:0.2rpx，在我的小米14手机上，会被转换为1px。400/750*0.2等于0.106，即大于0且小于1时，会向上取整。
#### 问题1-03：给标签设置left:0.8rpx，在我的小米14手机上，会被转换为1px。400/750*0.8等于0.426，即大于0且小于1时，会向上取整。
#### 问题1-04：给标签设置left:2.8rpx，在我的小米14手机上，会被转换为1px。400/750*2.8等于1.493，即大于1时，会向下取整。
#### 问题1-05：给标签设置left:3.2rpx，在我的小米14手机上，会被转换为1px。400/750*3.2等于1.706，即大于1时，会向下取整。
#### 问题1-06：给标签设置left:3.8rpx，在我的小米14手机上，会被转换为2px。400/750*3.8等于2.026，即大于1时，会向下取整。
#### 问题1-07：给标签设置left:-0.2rpx，在我的小米14手机上，会被转换为-1px。400/750*-0.2等于-0.106，即大于-1且小于0时，会向下取整。
#### 问题1-08：给标签设置left:-0.8rpx，在我的小米14手机上，会被转换为-1px。400/750*-0.8等于-0.426，即大于-1且小于0时，会向下取整。
#### 问题1-09：给标签设置left:-2.8rpx，在我的小米14手机上，会被转换为-1px。400/750*-2.8等于-1.493，即小于-1时，会向上取整。
#### 问题1-10：给标签设置left:-3.2rpx，在我的小米14手机上，会被转换为-1px。400/750*-3.2等于-1.706，即小于-1时，会向上取整。
#### 问题1-11：给标签设置left:-3.8rpx，在我的小米14手机上，会被转换为-2px。400/750*-3.8等于-2.026，即小于-1时，会向上取整。
#### 解决方案之wxss：如果是0，就不要附加rpx单位。如果不是0，则这些误差是可以接受的。
#### 解决方案之wxml：同上。或自行把rpx转换为px，使用screenWidth/750*目标值进行计算。

## 给微信小程序的自定义组件标签本身设置样式不生效，其本质原因是没把自定义组件标签本身设置为块元素或内联块元素。
#### 案例：`<BaseHelloWorld style="display: block;background: rgba(0,255,0,0.8);" />`。
#### 发现：上述如果没有display: block;则其他样式不会生效。
#### 总结：经过多次测试，我总结了以下解决方案。
* 方案1：把BaseHelloWorld这个标签主动变成块元素或被动变成块元素。
* 方案2：把BaseHelloWorld这个标签主动变成内联块元素。
* 方案3：把BaseHelloWorld这个组件内部的根节点变成内联元素或内联块元素。
* 方案4：给BaseHelloWorld这个组件外部的父级设置样式。
* 方案5：使用常规的组件通信方式把样式传递到组件内部并进行相关处理。
  - 通信时入参的名称不能是style，否则会无效。
  - 可以换个名称，例如把style换成cssStyle。

## 删除微信小程序
#### 删除微信小程序，微信小程序的本地存储能清理掉吗？
* 能！
#### 删除微信小程序，微信小程序内嵌的h5本地存储能清理掉吗？
* 不能！
* 不仅需要删除小程序！还需要清理微信缓存！

## app或小程序内嵌h5时，使用input调起相册。
#### 微信小程序内嵌h5
* h5 input的type为file 能调起相册吗？
  - 能！
  - ios需要额外处理，input需要在body中渲染。否则change事件不会触发。
#### app内嵌h5
* h5 input的type为file 能调起相册吗？
  - 能！
  - 亲测发现：飞书可以调起、微信可以调起、企业微信可以调起。
  - 调不起来的app，需要app自己反思反思。
* app内测分发平台：https://www.pgyer.com

## 微信公众号配置跳转小程序，配置完路径之后，出现了`Page not found`。
#### 配置的小程序路径不能带query参数，即不能带`?key=val`。

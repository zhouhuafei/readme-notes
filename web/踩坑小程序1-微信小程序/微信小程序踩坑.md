...
* 公用样式和字体使用wxss文件。建议都使用wxss文件。因为scss会把import的文件杂糅到页面中(即使是wxss文件)。
    ...
    - 引入的时候引入注释版本的wxss。
    ```
    /*@import "../../scss/_iconfont.wxss";*/
    /*@import "../../scss/_reset.wxss";*/
    ```
    - 打包的时候把指定的scss文件转成wxss文件。然后再把注释的释放掉。
    - 最优解：使用微信原生的wxss文件。因为使用原生wxss互相引用(@import)的话不会导致内容增加。
    - 最优解难点在于如何配合scss使用。这是我一直没有解决的问题。
* 如果非要使用scss。则建议```/*import 'xxx.wxss'*/;```的部分注释掉。当把scss转成css之后。再使用```gulp-batch-replace```替换掉注释。
* 压缩js。
* 不要使用很大的第三方工具包。

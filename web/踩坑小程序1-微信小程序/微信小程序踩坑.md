...
* 小程序的渲染层和逻辑层分别由`2`个线程管理：渲染层的界面使用了`WebView`进行渲染，逻辑层采用`JsCore`线程运行`JS`脚本。
  - js脚本逻辑是在`JsCore`中运行，`JsCore`是一个没有窗口对象的环境，所以不能在js脚本中使用BOM。
    - 不过微信小程序暴露了一些API`wx.getSystemInfo()`。可用于获取系统信息。
  - 微信小程序没有暴露DOM的API供`JsCore`驱使，固也无法在js脚本中操作DOM。
...
* 微信小程序的框架包含两部分`View`视图层和`App Service`逻辑层。
  ...
  - 视图层和逻辑层通过系统层的`JSBridge`进行通信。
    - JSBridge是啥？基础库么？待续...
* 在浏览器中js可以操作BOM和DOM是因为浏览器环境中有BOM和DOM且浏览器提供了BOM和DOM的API供JS引擎驱使。
  ...
  - 在浏览器中，javascript的三大组成部分是：ECMAScript、DOM、BOM。

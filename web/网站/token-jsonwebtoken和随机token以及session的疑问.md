...
* 退出登录之后怎么注销token？
    ...
    - 方案2：退出登录的时候把token拉入黑名单。例如放到redis中。并设置上剩余的过期时间。验证token是否有效的时候进行判断，如果redis中存在这个token。则表示这个token是无效的。
* jwt怎么刷新token？
    ...
    - 改：用refreshToken换取新的token。具体请参阅```./token-jsonwebtoken刷新.md```文件。
* 做单设备登录时才需要登录戳。不做单设备登录是不需要的。当然让token业务上失效也需要使用到登录戳。
...
* 使用session和第三方对接。第三方打我们的登录成功之后存储sessionid(在```response.headers['set-cookie']```中获取)。下次请求时带在请求头的cookie上。如果过期则重新登录。
    ...
    - 别人测试在微信小程序客户端的响应里使用```response.headers['set-cookie']```可以获取到'set-cookie'。手机中需要使用大写的```response.headers['Set-Cookie']```获取(本人保持怀疑态度)(经官方文档证实此法可行)。

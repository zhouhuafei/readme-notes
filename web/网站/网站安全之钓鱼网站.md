...
* 后端预防：
    ...
    SAMEORIGIN：iframe页面的地址只能为相同域名下的页面(域名一致不是主域一致，即：a.24678.top只能嵌套a.24678.top的页面，不能嵌套b.24678.top的页面)。
...
* 前端预防：
    ...
    ```
    - iframe跨子域：把iframe中的页面以及嵌入了iframe的那个页面的domain都设置为顶级域名。案例：```document.domain = '24678.top';```。

...
* 选择规格时，如果差一个规格项就选择完毕了或者每一个规格项都选择完毕了，则从无库存的组合项中进行无库存规格值查找。找到则置灰。
    ...
    - 差一个规格项没被选择，则找到没被选择的那一项的值即可。
    - 全部规格项都被选择了，先要查下无库存的组合项，被选中了几项，如果无库存的组合项，剩一项没被选中，则找到没被选中的那一项的规格值即可。
    - 最后对无库存规格项的规格值，统一进行置灰即可。
    - 置灰之前，要先设置为全部都可点击。
    - 总结：找无库存的组合项 - 从组合项中筛选出需要被置灰的那一项的规格值 - 统一置灰
* 不满足查找条件，则要设置全部都可点击。

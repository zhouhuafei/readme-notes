...
* import命令输入的变量都是只读的，因为它的本质是输入接口。也就是说，不允许在加载模块的脚本里面，改写接口。即：不允许对import导入的值进行改写。
    ...
    ```
* es6的模块化怎么导出一个匿名的json对象，就像commonjs规范那样直接```module.exoorts={}```导出？```export default {a: 1, b: 2};```怀疑此处有误待续...和wepy转taro踩坑有冲突
* es6的模块化怎么导出一个匿名的数组，就像commonjs规范那样直接```module.exoorts=[]```导出？```export default [1,2,3];```怀疑此处有误待续...和wepy转taro踩坑有冲突
* 非匿名导出案例：
```
// 错误
export 'str';

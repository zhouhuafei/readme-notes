...
* 二次封装方式2、优化axios的二次封装
    ...
    ```
* 是优化还是退化？请看下述```总结2```。
    - 答：退化。虽然Promise.all需要二次处理数据，但是因为和async/await的响应结果保持了统一且遇到需要请求完改变开关状态的场景，不需要多写一个catch进行处理。相对的，每条请求响应之后都需要判断状态是success才能继续执行业务逻辑。
...
* ```二次封装方式1、```优点：
    ...
    - 1、使用async/await和在Promise的then中都需要对数据进行二次状态判定。保持了统一。
* ```二次封装方式1、```弊端：
    - 1、Promise.all如果遇到失败的请求，在catch中捕获不到。需要在then中对数据进行二次状态判定。
    - 2、使用Promise触发请求时，无论成功和失败都会走到then中，每条请求都需要在then中对数据进行二次状态判定。
    
# 最终总结
* ```二次封装方式1、```才是正确的封装方式。因为应用场景更广泛。
    - 每个响应都应该判断是业务成功才继续走。
    - 使用Promise.all进行并发时，要对每一项进行状态的二次判定。这是没毛病的。
    - 因为使用async/await进行并发时，也需要以及应该对每一项进行状态的二次判定。

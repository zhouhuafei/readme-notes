...
* linux查看进程的运行明细：`ll /proc/1442`。1442是PID。
* linux查看哪些服务占用了80端口以及其进程的PID：`netstat -ntulp | grep 80`。或使用`lsof -i:80`。
  - win：`netstat -ano | findstr 80`。
...
* `-`：`node -v`
* `--`：`node --version`
> `--`还可以用来拼接参数。`--`后面的参数不解析，直接传递进去。
* `npm run lint -- --fix`

## top命令
> 使用top命令可以显示进程所占用的系统资源
* 在命令行中输入top命令并按下回车键可以显示top基本视图
* 按`shift + m`视图会按照内存的使用情况从大到小排序
* 按`shift + p`视图会按照CPU的使用情况从大到小排序
* 按`数字1`视图会列出所有核CPU的使用状态，再按一次`数字1`视图会恢复成以前的状态。
* 按`q`键可以退出top基本视图

## `;`和`&&`和`||`的区别
* `pwd ; ll`：不论前一个命令是否执行成功，后一个命令都会执行。
* `pwd && ll`：前一个命令执行成功，后一个命令才会执行。
* `pwd || ll`：前一个命令执行失败，后一个命令才会执行。

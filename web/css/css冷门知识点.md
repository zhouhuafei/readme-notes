...
* 兄弟节点，如果div1使用fixed进行定位，默认不加z-index的情况下，会覆盖掉div2，但是只要给div2加上relative或absolute或fixed，则div2就不会被覆盖了。
  ...
  - 知识点：子元素设置fixed层级z-index对比。
    ...
    - 如果两个父元素没有定位属性或有relative/absolute但没有z-index属性，那么才会轮到子元素自身的z-index的对比。
  - 案例：A元素fixed、B元素absolute、谁层级高谁就会覆盖谁。
    - 如果B元素的父级有relative/absolute，没有z-index。A和B，谁层级高谁就会覆盖谁。
    - 如果B元素的父级有relative/absolute，有z-index。A和B元素的父级，谁层级高谁就会覆盖谁。
    - 如果B元素的父级有fixed，不管有没有z-index。A和B元素的父级，谁层级高谁就会覆盖谁。
* 定位时的层级z-index

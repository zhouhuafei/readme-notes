...
#### 知识点补充：浏览器刷新时，强缓存会失效吗？
* 什么是首条请求？
  - 顾名思义，首条请求就是第一条请求。
  - 案例1：例如你把某个网页的地址，贴入到浏览器的地址栏中，对index.html页面发起的请求就是首条请求。
  - 案例2：例如你把某个远程css文件，贴入到浏览器的地址栏中，对这个css发起的请求就是首条请求。
* 触发了浏览器的刷新行为，首条请求的强缓存会失效（失效后，有弱缓存则返回304，无弱缓存则返回200 OK）。例如点击浏览器的刷新按钮或通过js进行页面刷新。
  - 注意1：在地址栏贴入已访问过的网址，并不一定会触发浏览器的刷新行为。
    - 假设你访问过A网页，则A网页的index.html会被浏览器进行强缓存。
    - 此时，如果你新开一个或多个tab，并在地址栏逐个贴入A网页的链接，然后逐个进行回车，则首条请求的强缓存依然生效。
    - 但是，如果你对tab的地址栏进行的是非首次回车，则首条请求的强缓存立即失效。
...
* 清除七牛的缓存方案2：给url上加个`?t=时间戳`使之溯源。此方案是清理各种缓存的通用解决方案，不仅能直接绕开本地缓存，还能规避CDN缓存，我个人强烈推荐。
#### 知识点补充：清理缓存？
* 清理缓存方式1：给url上加`?t=时间戳`。不管是七牛服务器的缓存还是浏览器的本地强缓存，都可以直接规避掉。
* 清理缓存方式2：给html的响应头设置Cache-Control为no-cache。仅适用于规避本地缓存（且需要等待上次缓存失效），无法规避CDN缓存。

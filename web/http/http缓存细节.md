...
* 场景2：只让`Last-Modified`生效或只让`ETag`生效或让两者同时生效时。
  ...
  - 自我答疑：Chrome浏览器特性如此，有弱缓存时会默认生效强缓存（默认生效的强缓存有时效性，大约半小时会失效一次）。若不想要这个特性，可以参考`场景3`。
...
* 除非你去七牛后台手动清除对应文件的缓存。或者给url上加个`?t=时间戳`使之溯源。
* 本地的`200 OK (from memory cache)`特性和上述一致。
#### 知识点补充：清理缓存？
* 清理缓存方式1：给url上加`?t=时间戳`。不管是七牛服务器的缓存还是浏览器的本地强缓存，此方案一定能清理掉，固可以放心使用。
...
* 问：子页面为什么会被强缓存？
  - 答：Chrome浏览器特性如此，有弱缓存时会默认生效强缓存（默认生效的强缓存有时效性，大约半小时会失效一次）。
* 问：强刷父页面能去除子页面的缓存么？
  - 答：若iframe是通过js进行渲染的，不管是同步渲染还是异步渲染，强刷均无用。
  - 答：若iframe是通过html直接渲染的，强刷有用。即使有用，让用户去强刷页面也是不合理的行为。
* 问：能去给nginx配置`Cache-Control no-cache;`，让子页面的强缓存无效么？
  ...
  - 若其他非html的静态资源都在第三方的cdn服务器上，nginx中只有index.html页面，那直接对根路径进行无强缓存配置即可。
* 问：能去给子页面的index.html文件加meta标签，使之`Cache-Control`失效么？
  - 答：可以加，但是为时晚矣。因客户那边已经被缓存住了！
  - meta去除强缓存，建议项目启动初期就加上，防止index.html被强缓存。

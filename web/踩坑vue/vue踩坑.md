...
# 组件回显
* 通过props传入一个默认值(defaultValue)即可，然后监听(watch)这个默认值改变(默认这个值为空，一般都是ajax之后这个默认值才会有数据)，则修改组件内部data上对应的数据。
    - 默认值不要和组件change时对外输出的数据产生关联，防止重复触发watch。
    - 应该在组件再次显示时，手动处理默认值使之改变。
    - 所以不要使用computed产出默认值，因为如果组件change之后，对外输出了一个值改变了data上的某个数据，而你又依赖了data上这个数据，则会重复触发watch。
    - 如果不在watch中监听并回显，而在created钩子中回显的话，是不行的，因为created只会触发一次，除非外部使用v-if处理组件，才可以多次触发created钩子。
* 怎么防止组件回显多次触发对默认值的监听。
    - 像弹窗这种，直接监听监听组件是否显示。然后处理默认值。测试默认值是否改变。
* 待续...这里要重新屡清楚。

...
* 怎么配置跨域？
    ...
    devServer: {
        proxy: {
            '/api': {
                target: 'http://24678.top/admin/api',
                changeOrigin: true,
                ws: true,
                pathRewrite: {
                    '^/api': ''
...
* 建议：只设置一个publicPath。这才是最优解。
    ...
    - 文件如果想挪到指定目录，可以使用nodejs写个脚本拷贝到对应目录。如此也可以解决favicon.ico文件无法换目录的问题。
    - 例如test环境打包出来的文件全部都放到dist/test目录下，因vue-cli对外暴露的参数配不出预期的效果，所以我才想要使用nodejs写个脚本做这些事。
    - 文件拷贝可以使用```vinyl-fs```模块(我从gulp包的依赖中找到的这个包)。可以用正则的方式批量拷贝文件。
        - 此方案是在本地把所有环境的包都生成好，然后再一次性上传到服务器。
        - 具体点的说法就是，在dist文件中生成test目录，生成alpha目录，然后再一次性把本地dist目录里所有的东西都上传到服务器的dist目录里。
    - 不想拷贝的话，可以直接使用```scp2```模块，把整个dist目录里的文件上传到服务器对应的目录

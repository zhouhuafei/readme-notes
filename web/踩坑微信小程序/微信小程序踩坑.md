...
* 有open_id就算是登陆了。但是如果获取不到userInfo也是问题。
    - 所以正确逻辑应该是有open_id以及userInfo才算登录。
    - 封装login方法，在里面获取open_id以及userInfo。
    - 分装isLogin方法，检测到open_id以及userInfo就算是登陆了。
    - 处理wx.getUserInfo方法。如果获取用户信息失败，就移掉某个关键字段(open_id或者userInfo)，让isLogin方法通不过。

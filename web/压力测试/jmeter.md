...
#### 如何根据测试报告。推测系统能支撑多少人瞬间访问？
> 并发测试：找到最佳的并发峰值。只要不是太烂的服务器，线程数与tps是成正比的，比例下降前的那个线程数就是最佳线程数。
>> 当线程数高于单台压测机限制时需进行分布式并发测试。
#### 如何根据测试报告。推测系统能支撑多少人长期访问？
> 压力测试：在最佳的并发峰值下进行长期运行。若接口的错误率低于1%，服务器和数据库的cpu以及内存使用率低于80%时，则可视为服务器运行稳定。
>> 当线程数高于单台压测机限制时需进行分布式压力测试。
...
#### jmeter报错
* 问题1：并发不高，但是请求对应的响应依然出错了。公司的电脑和网络不会出现下述错误。家里的电脑和网络会出现下述错误。
  - org.apache.http.conn.ConnectTimeoutException
  - java.net.SocketTimeoutException: Read timed out
  - java.net.SocketException: Connection reset
  - 按照网上的教程，试过改超时时间以及改注册表。重启后然并卵：改了屁用没有。该报错还是报错。
* 问题2：压测次数太频繁，之前线程对应的端口还没释放，又开启了下次压测，导致端口不够用。这个问题的原因是windows端口被耗尽了（默认1024-5000），而且操作系统要2~4分钟才会重新释放这些端口，所以可以增加windows的可用端口来解决，windows端口最大数为65535。
  - address already in use
  - 按照网上的教程，试过扩大端口数。重启后然并卵：改了屁用没有。该报错还是报错。

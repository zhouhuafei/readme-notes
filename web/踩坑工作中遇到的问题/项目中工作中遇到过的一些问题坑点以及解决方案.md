...
.iPhoneXh-safe {
  ...
  //如果使用padding还需要考虑不支持的场景，不支持时，还不能把默认的5px弄丢了。
  //如果封装成mixin使用就会变的灵活。
...
* 场景3：服务没启动。
  ...
  - `nginx`超时会报`504 Gateway Timeout`。默认1分钟超时。`axios`会报`Request failed with status code 504`。
  - 如果涉及到跨域且跨域不是配置在`nginx`里的，不同域时，上述`502`和`504`会导致`axios`的请求跨域，此时`axios`会报`Network Error`。
...
console.log(JSON.parse(JSON.stringify(e))) // {a: 1, b: 2, message: "error message"}
```
* 案例示范：
```javascript
try {
  let e = new Error('error message')
  Object.assign(e, {a: 1, b: 2})
  console.log(e.message) // error message
  throw e
} catch (e) {
  console.log(JSON.parse(JSON.stringify({ErrorMessage: e.message, e}))) // {ErrorMessage: "error message", e: {a: 1, b: 2}}
}
```
* 应用示范：
  - https://github.com/zhouhuafei/easy-tool-set_api/blob/master/src/utils/responseDataFormat.js
  - 在上述文件中搜索`ErrorMessage`关键字即可看到应用示范

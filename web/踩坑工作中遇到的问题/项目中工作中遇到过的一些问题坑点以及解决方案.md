...
* vue-router在IOS的微信内置浏览器中如果切换了页面。则右上角的复制链接一直是第一个页面的链接。
    ...
    - 引申问题：微信sdk，一直注册失败。
    - 问题原因：微信sdk只认第一个页面的链接。window.location.href获取到的值是当前页面的链接(非第一个页面的链接)。两者不一致导致的。微信sdk需要两者是一致的。
    - 解决方案：解决上述两个问题，只需要把router-link标签换成a标签即可。但是这种改法太恶心。所以应该在全局路由守卫中做文章。
    ```
    function wxRefresh (to) {
      if (to.query.wxr || !window.navigator.appVersion.match(/iphone/ig)) { // 如果已经触发过了replace 或者 如果是非IOS则不做处理。
        return
      }

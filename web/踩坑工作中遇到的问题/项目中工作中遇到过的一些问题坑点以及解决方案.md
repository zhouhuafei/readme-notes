...
* 七牛云使用-图片基本处理：`?imageView/1/w/240`。
  - 官方文档：https://developer.qiniu.com/dora/api/1279/basic-processing-images-imageview2
  - 使用案例：`https://qiniu.icaodong.com/image/573_1570587523424_314813416.jpg?imageView/1/w/240`。
* 七牛云上传的图片翻了90度。
...
```
* 案例：
  - https://github.com/zhouhuafei/easy-tool-set_api/blob/master/src/utils/responseDataFormat.js
  - 在上述文件中搜索`toJSON`关键字
...
* 我家里MAC电脑分辨率是：`2560*1600`。dpr是`2`。网页宽度是...TODO。外接了我家里的显示屏。按照4500尺寸的psd切图，公告图在小屏上超级清晰，在大屏上也超级清晰。
* 我家里ThinkPad电脑分辨率是：`1920*1080`。dpr是`1.5`。网页宽度是...TODO。外接了我家里的显示屏。按照4500尺寸的psd切图，公告图在小屏一般清晰，在大屏上也一般清晰。
* 我公司DELL电脑分辨率是：`2560*1440`。dpr是`1`。网页宽度是2560px。未外接大屏。按照4500尺寸的psd切图，公告图在屏幕上显示时，比ThinkPad小屏清晰，没MAC小屏清晰。
* 我同事MAC电脑分辨率是：...TODO。
...
* 方案以及建议：建议整张psd切成一整张图，然后全屏观察这张图，如果在自己的设备上看不清，那么在大屏上观看大概率也是看不清的。
* 通俗一点解释：想要看清一块本身不糊的内容，要么是这块内容本身就很大（让公告占用更大的区域），要么是你使用放大镜去观察（使用更大的屏幕/更高的分辨率/更贵的设备/前提是页面使用vw单位进行自适应布局）。

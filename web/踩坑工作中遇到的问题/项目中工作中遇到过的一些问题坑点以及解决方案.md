...
* blur事件要打接口(异步)验证值是否合法，但是刚好却偏偏下一步遇上了点击提交表单，导致拿不到blur之后的值。
    ...
    - 解决方案3：使用input事件配合函数去抖。（体验并不是很完美，稍微怪异，我输入的如果比较慢，提示就稍显频繁。如果遇到需要重新覆盖input值的场景，体验更是怪异，我输入值或者删除值时，因接口异步所以难免会遇到值被覆盖或清除的状况，去抖的时间间隔200-500无论怎么调整都不完美）。
    - 解决方案4：失去焦点直接打接口验证(开个定时器)，当点击了下单，清掉定时器，打验证接口，验证通不过给错误提示，验证通过打下单接口。(失去焦点验证了一次，下单还要打接口再验证一次)。

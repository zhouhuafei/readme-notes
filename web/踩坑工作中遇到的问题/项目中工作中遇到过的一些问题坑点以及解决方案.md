...
## 公告图中的文字在大屏上无法清晰展示？
* 我家里MAC电脑分辨率是：`2500*1600`。dpr是2，外接显示屏的dpr也会是2（...TODO）。按照4500的分辨率切图，公告图在小屏上超级清晰，在大屏上也超级清晰。
* 我家里ThinkPad电脑分辨率是：`1920*1080`。dpr是1.5，外接显示屏的dpr也会是1.5（...TODO）。按照4500的分辨率切图，公告图在小屏一般清晰，在大屏上也一般清晰。
* 我公司DELL电脑分辨率是：`2560*1440`。dpr是1。没有外接显示屏。按照4500的分辨率切图，公告图比在我家里的ThinkPad电脑上要清晰。
* 我家里大屏设备分辨率是（建议按照大屏的设备分辨率制作设计图）：`3840*2160`。27英寸。4K屏。调成4096分辨率试试？...TODO
* 设计图的分辨率是：`9000*5062` 建议设计图的尺寸和设备的尺寸保持一致，或是设备尺寸的2倍（足以），此处建议设计图的尺寸最大不要超过7680*4320大小。已足以应对大多数屏幕，因为大多数屏幕的设备物理像素比是不超过2的。
...
* 通俗一点解释：想要看清一块区域，要么是这块区域本身就很大（让公告占用更大的区域），要么是你使用放大镜去观察（使用更大的屏幕/更高的分辨率/更贵的设备）。
* 设计是按照9000分辨率出的设计图，设计图尺寸为9000*5062，切出来的图，即使[压缩](https://tinypng.com/)后依然很大(压缩后单张背景图不超过2M，单张公告图不超过800KB)，会导致轮播图切换卡顿。
  ...
  - pc页面，按照1920尺寸进行设计，文字最小是12px（浏览器限制）。那么若按照9600尺寸出设计图（5倍图），则文字至少要60px（若小于这个值，等比缩小到1920后一定看不清）。
  - h5页面，按照375尺寸进行设计，文字最小是12px（浏览器限制）。那么若按照750尺寸出设计图（2倍图），则文字至少要24px（若小于这个值，等比缩小到375后一定看不清）。

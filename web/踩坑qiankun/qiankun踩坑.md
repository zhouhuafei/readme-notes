...
* 注：建议使用主子应用通信的方式，因为注册只发生在初始化的时候。
  - 子应用加载完毕后，如果主应用数据发生了变更，子应用无法感知。
...
* 子应用应该使用自己的store！自己打接口获取属于自己的数据！自己维护自己的数据！不应该和主应用强耦合！

## 主应用和子应用怎么通讯？
* 方案1：在主应用中，使用订阅发布模式，建立一套全局的通信规则。
* 方案2：使用官方提供的通信机制。本质也是订阅发布。

...
* 解决问题：临时的解决方案是，我改为以mac系统为基准进行开发。终极解决方案是统一win系统和mac系统的字体，统一字体后，如果测量出来的宽度是小数则进行向上取整，否则Safari浏览器无法兼容。
...
* 总结经验：统一字体后，经过多组数据测试后发现，Chrome浏览器在两个系统上的误差可以忽略，Safari浏览器针对小数是四舍五入，所以当测量出来的结果是小数时需要进行向上取整，否则Safari浏览器无法兼容。
